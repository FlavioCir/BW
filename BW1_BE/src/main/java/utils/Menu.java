...
public class Menu extends JpaUtils {
	...
	public static void scegliMezzo() {		
		...
		for(int i = 0; i < aut.size(); i++) {
			Autobus a = aut.get(i);
			System.out.println("A" + (i+1) + " - " + a.getTipoMezzo() + " " + a.getNumero() + ANSI_YELLOW + " direzione --> " + ANSI_RESET + a.getPartenza() + " - " + a.getCapolinea());
		}
		for(int i = 0; i < tr.size(); i++) {
			Tram t = tr.get(i);
			System.out.println("T" + (i+1) + " - " + t.getTipoMezzo() + " " + t.getNumero() + ANSI_YELLOW + " direzione --> " + ANSI_RESET + t.getPartenza() + " - " + t.getCapolinea());
		}
